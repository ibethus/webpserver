...
public class ImageResource {
    ...
    @RunOnVirtualThread
    @Operation(
        operationId = "uploadImageMultipart",
    ...
    @APIResponses({
        @APIResponse(responseCode = "201", description = "Image successfully uploaded and converted.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = ImageService.UploadResult.class))),
        @APIResponse(responseCode = "400", description = "Unsupported image format or file exceeds size limit.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
        ...
        @APIResponse(responseCode = "429", description = "Upload rate limit exceeded.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = ErrorResponse.class))),
        @APIResponse(responseCode = "500", description = "Internal error during conversion or storage.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = ErrorResponse.class)))
    })
    ...
    @APIResponses({
        @APIResponse(responseCode = "201", description = "Image successfully fetched and converted.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = ImageService.UploadResult.class))),
        @APIResponse(responseCode = "400", description = "Missing or empty `url` field, or unsupported image format.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = ErrorResponse.class))),
        @APIResponse(responseCode = "401", description = "Missing or invalid Bearer token.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = ErrorResponse.class))),
        @APIResponse(responseCode = "422", description = "URL unreachable, returns non-2xx, or content is not a supported image format.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = ErrorResponse.class))),
        @APIResponse(responseCode = "429", description = "Upload rate limit exceeded.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
    ...
    @APIResponses({
        @APIResponse(responseCode = "200", description = "Image content.",
            headers = {
                @Header(name = "X-Cache",
                    description = "`HIT` if served from cache (in-memory index or disk), `MISS` if generated on this request.",
                    schema = @Schema(type = SchemaType.STRING, enumeration = {"HIT", "MISS"})),
                @Header(name = "Cache-Control",
                    description = "`public, max-age=31536000, immutable` for cached variants; `public, max-age=3600` for originals.",
                    schema = @Schema(type = SchemaType.STRING))
            },
            content = @Content(mediaType = "image/webp",
                schema = @Schema(type = SchemaType.STRING, format = "binary"))),
        @APIResponse(responseCode = "400", description = "Size not in `VALID_SIZES`, or resize requested on an animated WebP.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = ErrorResponse.class))),
        @APIResponse(responseCode = "404", description = "Image not found.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = ErrorResponse.class))),
        @APIResponse(responseCode = "500", description = "Internal error during decoding, resizing or encoding.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = ErrorResponse.class)))
    })
    public Response getImage(
            ...
            @Parameter(
                ...
                in = ParameterIn.PATH,
                required = true,
                description = "Filename returned by the upload endpoint. Must be `{uuidv4}.webp`.",
                example = "3f2a1b4c-8e7d-4f6a-9b2c-1d3e5f7a9b0c.webp",
                schema = @Schema(type = SchemaType.STRING, pattern = "^[a-f0-9\\-]{36}\\.webp$"))
            String filename,
            @QueryParam("w")
            @Parameter(name = "w", in = ParameterIn.QUERY, description = "Target width in pixels.",
                schema = @Schema(type = SchemaType.INTEGER, minimum = "1", maximum = "10000"))
            Integer w,
            @QueryParam("h")
            @Parameter(name = "h", in = ParameterIn.QUERY, description = "Target height in pixels.",
                schema = @Schema(type = SchemaType.INTEGER, minimum = "1", maximum = "10000"))
    ...
    @Operation(
        ...
        summary = "Delete an image and all its cached variants",
        description = """
    ...
    @SecurityRequirement(name = "BearerAuth")
    @APIResponses({
        @APIResponse(responseCode = "200", description = "Image and all variants successfully deleted.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = ImageService.DeleteResult.class))),
        @APIResponse(responseCode = "401", description = "Missing or invalid Bearer token.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = ErrorResponse.class))),
        @APIResponse(responseCode = "404", description = "Image not found.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = ErrorResponse.class))),
        @APIResponse(responseCode = "500", description = "Unexpected error during deletion.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                schema = @Schema(implementation = ErrorResponse.class)))
    })
    public Response deleteImage(
            @PathParam("filename")
            @Parameter(name = "filename", in = ParameterIn.PATH, required = true,
    ...
    public record UploadUrlRequest(
        @Schema(description = "Publicly accessible URL of the image to fetch and store.",
                example = "https://example.com/photo.jpg", format = "uri")
        String url) {}
